...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/16 11:31:03 by zsailine          #+#    #+#             */
/*   Updated: 2024/07/16 13:21:47 by zsailine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_katana1(int j, int i, t_vars *game)
{
	static int	tmp;

	if (tmp % 2 == 0)
		draw_image1(j, i, game->katana_1.img, game);
	else
		draw_image1(j, i, game->katana_2.img, game);
	tmp++;
}
	...
	while (game->map->map[i])
	{
		...
		while (game->map->map[i][j])
		{
			if (game->map->map[i][j] == 'P')
				draw_image1(j, i, game->zorro_f1.img, game);
			if (game->map->map[i][j] == '0')
				draw_image1(j, i, game->ground.img, game);
			if (game->map->map[i][j] == '1')
				draw_image1(j, i, game->wall.img, game);
			if (game->map->map[i][j] == 'C')
				draw_katana1(j, i, game);
			if (game->map->map[i][j] == 'E')
				draw_image1(j, i, game->portal.img, game);
			j++;
		}
		i++;
	}
